...
class SwitchParentage(Move):
    ...
    def execute(self, model : Model) -> Model:
        ...
        edge_2_remove : list[Node] = self.random_object(net.inEdges(node_2_change), model.rng)
       ...
       # # print(f"STARTING AT CUR_CT: {cur_ct}")
        while cur_ct != target:
            ...
            # if changing_retic:
            #     #print({key: [(node[0].get_name(), node[1].get_name()) for node in values] for (key, values) in edges_to_ct.items()})
            #     #changing_retic = False
            #     pass
    ...
    def undo(self, model : Model)-> None:
        ...
        if self.undo_info is not None:
            ...
            # if self.print_net:
            #     print("REJECTING")
            #     print(model.network.to_newick())
            #     model.network.pretty_print_edges()
            #     model.network.print_graph()
            #     print({node.get_name() : degree for (node, degree) in model.network.in_degrees.items()})
            #     print({node.get_name() : degree for (node, degree) in model.network.out_degrees.items()})
    ...
    def delete_edge(self, net:DAG, edge:list[Node]):
        ...
        while len(q) != 0:
            ...
            if len(neighbors) == 2 and neighbors[0].get_name() == neighbors[1].get_name() and bypass:
                ...
                if len(b) != 0:
                    net.removeEdge([b[0], neighbors[0]])
                    net.removeEdge([neighbors[0], cur])
                    net.removeEdge([neighbors[1], cur])
                    net.addEdges([b[0], cur])
                else:
                    net.removeEdge([neighbors[0], cur])
